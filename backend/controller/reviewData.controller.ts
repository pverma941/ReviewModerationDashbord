...
const computeRisk = async ( textContent: string, productId: string, author: string, rating: number ): Promise<number> => {
      let score = 50;
      ...
      let bannedHits = 0;
      for (const w of banned) if (lowered.includes(w)) bannedHits++;
      score += bannedHits * 12;
      ...
      if (excl >= 5) score += 18;
      else if (excl >= 3) score += 10;

      // 4) very short or meaningless text
      if (textContent.length < 10) score += 25;
      else if (textContent.length < 30) score += 8;

      // 5) repeated characters (e.g., loooove)
      if (/(.)\1{3,}/.test(textContent)) score += 12;
      ...
      if (typeof rating === "number") {
        if (rating <= 2) score += 15;
        else if (rating === 3) score += 6;
        else if (rating === 5) score -= 8;
      }
...
export const createReviewData = async (req: Request, res: Response) => {
  try {
...
export const getAllReviewData = async (req: Request, res: Response) => {
  try {
...
export const updateReviewData = async (req: Request, res: Response) => {
  try {
    ...
    if (Object.keys(updateFields).length === 0){
      ...
      if (!existing) return res.status(404).json({ error: "Review data not found" });   
      ...
      if (!reviewData) return res.status(404).json({ error: "Review data not found" });
      return res.status(200).json({
...
export const approveReviewData = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    if (!id) return res.status(400).json({ error: "ID is required" });
    ...
    const result = await ReviewData.updateOne(
      { productId: id }, 
      { $set: { status: "approved" } },
      { runValidators: true } 
    );
...
export const rejectReviewData = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    if (!id || !req.body.reason) return res.status(400).json({ error: "incomplete data" });
...
export const getFlaggedReviewData = async (req: Request, res: Response) => {
  try {
